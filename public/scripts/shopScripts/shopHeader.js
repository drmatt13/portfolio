...
const cartCircleContainer = document.getElementById("cart-circle-container");
const cartCircleCounter = document.getElementById("cart-circle-counter");
...
const pictureBanner = document.getElementById("pictureBanner");
window.addEventListener("scroll", () => {
  ...
  if (window.pageYOffset < pageYOffset && window.pageYOffset < spacer && !navbarVisable) {
    ...
    pictureBanner.classList.remove("picture-banner-margin");
  }
  if (window.pageYOffset < pageYOffset && window.pageYOffset >= spacer && window.pageYOffset <= delayEndUp) {
    navbar.classList.add("navbar-scrollUp");
    navbar.classList.remove("navbar-scrollDown");
    pictureBanner.classList.add("picture-banner-margin");
    navbarVisable = true;
  }
  if (window.pageYOffset > pageYOffset && window.pageYOffset < spacer && !navbarVisable){
    navbar.classList.add("navbar-relative");
...
const openCart = () => {
  ...
  if (!isMobile) {
    ...
    pictureBanner.classList.add("padding-right");
...
const closeCart = () => {
  ...
  if (!isMobile) {
    ...
    pictureBanner.classList.remove("padding-right");
...
const updateCart = () => {
  if (!promiseFulfilled) {
    ...
    for (let i=0; i<localStorage.length; i++) {
      itemTotal += +localStorage.getItem(localStorage.key(i));
    ...
    if (itemTotal == 0) {
      cartBtn.innerHTML = `
      <div class="cart-btn">
        <div class="fas fa-shopping-cart black"></div>
      </div>`;
    } else {
      cartBtn.innerHTML = `
      <div class="cart-btn">
          <div class="fas fa-shopping-cart"></div>
          <div id="cart-circle-container" class="cart-circle-container"></div>
          <div id="cart-circle-counter" class="cart-circle-counter">${itemTotal}</div>
      </div>`;
    }
    updateSubtotal();
  }
